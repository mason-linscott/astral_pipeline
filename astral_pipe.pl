...
my %opts;
getopts( 'ab:c:g:hi:l:m:M:o:p:P:rs:u', \%opts );
...
my( $loci, $map, $proc, $groupsize, $astralonly, $boot, $constrain, $version, $remove, $memory, $pis, $snps ) = &parsecom( \%opts );
...
opendir( WD, $grouped_locidir ) or die "Can't open $grouped_locidir, d-bag: $!\n";
...
foreach my $file( @contents ){
	if( $file =~ /\.phy$/ ){
		...
		&runraxml( $proc, $file, $raxmldir, $grouped_locidir, $boot );
		next;
	}
}
...
sub parsecom{ 
  ...
  return( $loci, $map, $proc, $groupsize, $astralonly, $boot, $constrain, $version, $remove, $memory, $pis, $snp );
...
sub pyloci{
	...
	open( LOCI, $loci ) or die "Can't open $loci, d-bag: $!\n";
	while( my $line = <LOCI> ){
		# read each line
		if( $line =~ /^>/ ){
		chomp( $line );
	...
	foreach my $locus( @genes ){
		...
		open( OUT, '>', $outdir ) or die "Can't write $outdir, d-bag: $!\n";
		#print "Writing $out\n";
...
sub removeMissing{
	...
	open( PHY, $file ) or die "Can't open $file, d-bag: $!\n";
	...
	open( KEPT, '>', $keptfile ) or die "Can't open $file, d-bag: $!\n";
...
sub parsemap{
	...
	open( MAP, $map ) or die "Can't open $map, d-bag: $!\n";
	...
	open( NEWMAP, '>', $astral_specmapdir ) or die "Can't open $astral_specmapdir, d-bag: $!\n";
...
sub grouploci{
	...
	for( my $i=0; $i<$full; $i++ ){
		...
		for( my $j=0; $j<$groupsize; $j++ ){
			...
			open( INFILE, $inputdir ) or die "Can't open $inputdir, d-bag: $!\n";
		...
		open( OUTPUT, '>', $groupnamedir ) or die "can't open $groupnamedir, d-bag: $!\n";
		# print header
...
sub runraxml{
	my( $proc, $file, $raxmldir, $grouped_locidir, $boot ) = @_;
	...
	system( "mpirun -np $proc -machinefile $ENV{'PBS_NODEFILE'} raxmlSSE3 -p 1234 -s $filepath -n $name -m GTRGAMMA -w $raxmldir -# $proc" ); # == 0 or die "ERROR: RAxML exited with non-zero status: $?";
	
	# run bootstrapping if requested
	if( $boot > 0 ){
		$name = join( '_', $name, "boot" );
		system( "mpirun -np $proc -machinefile $ENV{'PBS_NODEFILE'} raxmlSSE3 -x 1234 -p 1234 -# $boot -s $filepath -n $name -m GTRGAMMA -w $raxmldir" )
	}
...
sub treecat{
	...
	opendir( RAX, $raxmldir ) or die "Can't open $raxmldir, d-bag: $!\n";
	...
	foreach my $file( @contents ){
		...
		if( $file =~ /^RAxML_bestTree\.\d+$/ ){
			open( TREE, $filedir ) or die "Can't open $filedir, d-bag: $!\n";
			while( my $line = <TREE> ){
				chomp( $line );
				push( @trees, $line );
			}
	...
	open( OUT, '>', $treeoutdir ) or die "Can't open $treeout, d-bag: $!\n";
...
sub bspaths{
	...
	open( BS, '>', $bspath ) or die "Can't open $bspath, d-bag: $!\n";
...
sub astralparse{
	...
	open( TREES, $astraloutdir ) or die "Can't open $astraloutdir, d-bag: $!\n";
	while( my $line = <TREES> ){
		chomp( $line );
		push( @trees, $line );
	}
	...
	my $best = pop( @trees );
	open( BEST, '>', $besttree ) or die "Can't open $besttree, d-bag: $!\n";
	print BEST $best, "\n";
	close BEST;
	
	# remove next to last item of array (astral bootstrap consensus tree) and print to new file
	my $aboot = pop( @trees );
	open( BOOT, '>', $bootcon ) or die "Can't open $bootcon, d-bag: $!\n";
	print BOOT $aboot, "\n";
	close BOOT;
	
	# print remaining trees (bootstrap trees) to new file
	open( BOOTS, '>', $boottrees ) or die "Can't open $boottrees, d-bag: $!\n";
	foreach my $tree( @trees ){
		print BOOTS $tree, "\n";
	}
	close BOOTS;
